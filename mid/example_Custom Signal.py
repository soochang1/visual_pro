...
class MW(QMainWindow): #1번 QMainWindow는 QObject의 subclass임.
    ...
    def init_ui(self):
        ...
        self.fstr = os.path.dirname(
            os.path.abspath('c:/visual_pro/number_image.png')
        )
    ...
    def setup_main_wnd(self):
        ...
        pixmap = QPixmap(f"{self.fstr}/number_image/0.png")
    ...
    def change_pixmap_handler(self, offset):
        ...
        if self.idx <0 :
            self.idx = 9
        print(self.idx)
        pixmap = QPixmap(f"{self.fstr}/number_image/{self.idx}.png")
        self.img_label.setPixmap(pixmap.scaled(
            QSize(180,250),
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation
        ))
        if self.idx == 0 or self.idx == 9:
            alert = QLabel("<p>끝에 도달하셨습니다!</p>")
            lm = self.centralWidget().layout()
            lm.addWidget(alert)
